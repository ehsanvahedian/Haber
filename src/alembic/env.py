...
def run_migrations_offline_with_url(url) -> None:
    context.configure(
        url=url,
        target_metadata=target_metadata,
        literal_binds=True,
...
if second_database_url and context.is_offline_mode():
    for url in [database_url, second_database_url]:
        run_migrations_offline_with_url(url)
